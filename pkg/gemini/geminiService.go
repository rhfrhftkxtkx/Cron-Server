...
func (geminiClient *Client) Processing(ctx context.Context, url *url.URL, webPage string, depth int) (*[]Response, error) {
	...
	baseUrl := url.Scheme + "://" + url.Hostname()
	content := fmt.Sprintf(prompt, geminiClient.BasePrompt, baseUrl, depth, webPage)
